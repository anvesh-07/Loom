...
const Search = ({ workspaceId }: Props) => {
  const { query, onSearcQuery, isFetching, onUsers } = useSearch(
    "get-users",
    "USERS"
  );
  // Work In Progress : Wire up sending the invitations
  ...
  return (
    <div className="flex flex-col gap-y-5 ">
      <Input
        onChange={onSearcQuery}
        value={query}
        className="bg-transparent border-2 outline-none"
        placeholder="Serch for your users..."
        type="text"
      />
      {isFetching ? (
